...
macroRescanTests =
  testGroup
    ...
    [ testCase "self-referential object macro expands exactly once" $
        assertPreprocessOutput
          (T.unlines ["#define SELF SELF", "e = SELF"])
          (T.unlines ["#line 1 \"<input>\"", "", "e = SELF"]),
      testCase "self-referential object macro keeps surrounding tokens" $
        assertPreprocessOutput
          (T.unlines ["#define SELF x + SELF", "e = SELF"])
          (T.unlines ["#line 1 \"<input>\"", "", "e = x + SELF"]),
      testCase "self-referential function macro expands exactly once" $
        assertPreprocessOutput
          (T.unlines ["#define REC(x) REC(x)", "f = REC(1)"])
          (T.unlines ["#line 1 \"<input>\"", "", "f = REC(1)"]),
      testCase "mutually recursive function macros terminate" $
        assertPreprocessOutput
          (T.unlines ["#define PING(x) PONG(x)", "#define PONG(x) PING(x)", "g = PING(2)"])
          (T.unlines ["#line 1 \"<input>\"", "", "", "g = PING(2)"]),
